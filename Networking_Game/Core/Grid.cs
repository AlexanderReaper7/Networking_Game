...
namespace Networking_Game
{
    ...
    public struct GridSquare
    {
        ...
        {
            if (IsInLine) spriteBatch.DrawFilledRectangle(position.X - gridLayout.LineThickness / 2, position.Y + gridLayout.LineThickness / 2, gridLayout.SquareSize, gridLayout.SquareSize, new Color((uint) Owner.Color));
            else Owner?.Draw(spriteBatch, position, gridLayout);
        }

        public void Claim(Player player) { Owner = player; }

        public void SetIsInLine(bool value) { IsInLine = value; }
    }
    ...
    public struct GridLayout
    {
        ...
        public Rectangle CalculatePlayArea(int squaresX, int squaresY) { return new Rectangle((int) Math.Ceiling(Position.X), (int) Math.Ceiling(Position.Y), (int) Math.Ceiling(SquareSize * squaresX + LineThickness), (int) Math.Ceiling(SquareSize * squaresY + LineThickness)); }
    }
    ...
    public class Grid
    {
        ...
        public bool ClaimSquare(Point position, Player player)
        {
            try
            {
                ...
                if (Squares[position.X, position.Y].Owner == null)
                {
                    ...
                    Console.WriteLine($"{player.Name} Claimed square {position}", player.drawingColor.ToSystemColor());
                    // Check for line
        ...
        public List<Point> CheckForLine(Point checkPoint, Player owner)
        {
            ...
            foreach (Point point in directions)
            {
                ...
                {
                    foreach (Point p in points) Squares[p.X, p.Y].SetIsInLine(true);
                    // add to output
        ...
        bool? IsValidBranchPoint(Point branchPoint, Player owner)
        {
            try { return Squares[branchPoint.X, branchPoint.Y].Owner == owner && Squares[branchPoint.X, branchPoint.Y].IsInLine == false; }
            catch (Exception) { return null; }
        }

        void DrawSquares(SpriteBatch spriteBatch, GridLayout gridLayout, Camera2D camera)
        {
