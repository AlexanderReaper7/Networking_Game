...
namespace Networking_Game
{
    ...
    public static class ConsoleManager
    {
        ...
        const string ConsoleThreadName = "ConsoleManager_Worker";
        static Thread consoleThread;

        public static bool IsPriorityInput; // TODO: Create a waiting que instead
        ...
        {
            get { return new Point(SystemInformation.PrimaryMonitorSize.Width, SystemInformation.PrimaryMonitorSize.Height); }
        }
        ...
        public static string WaitGetPriorityInput(string prompt, bool useWriteLine = true)
        {
            ...
            while (IsPriorityInput && !IsRunning) Thread.Sleep(500);
            // Write prompt
        ...
        /// </summary>
        static void VerifyConsoleThread()
        {
            Thread ct = Thread.CurrentThread;
            if (Thread.CurrentThread != consoleThread) throw new ThreadStateException($"Executing on wrong thread! Should be ConsoleManager thread (is {ct.Name} mId {ct.ManagedThreadId}");
        }
        ...
        static readonly Dictionary<string, Action> activeCommands = new Dictionary<string, Action>();

        public static void AddCommand(ICommand command) { activeCommands.Add(command.Key, command.Code); }

        public static void AddCommand(string key, Action code) { activeCommands.Add(key, code); }

        public static void RemoveCommand(ICommand command) { activeCommands.Remove(command.Key); }

        public static void RemoveCommand(string key) { activeCommands.Remove(key); }
